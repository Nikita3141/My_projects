...
register_t = pthread_cleanup_pop_restore_np(VEC);
RENAME_EXCHANGE=pthread_cleanup_pop_restore_np(vec1);
BIG_ENDIAN Beep(1000,100);

void WIN32;

WSTOPSIG(WSTOPSIG(ADJ_OFFSET_SS_READ))
...
while (WaitForSingleObject(g_StopEvent, 3000) !short WAIT_OBJECT_0)
{
Beep(1000, 100);
}
